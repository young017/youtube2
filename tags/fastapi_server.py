...
# .env 파일 로드 (프로젝트 루트 디렉토리에서 찾기)
import pathlib
project_root = pathlib.Path(__file__).parent.parent  # tags 디렉토리의 부모 디렉토리 (프로젝트 루트)
    ...
    try:
        ...
        if df_2025_cache is None:
            ...
            if not youtube_api_key:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="YOUTUBE_API_KEY 환경 변수가 설정되지 않았습니다. .env 파일에 설정해주세요."
                )
